...
function drawSprite(ctx: CanvasRenderingContext2D, sp: Sprite, x: number, y: number) {
...
function makeChair(): Sprite {
	...
	for (let r = 0; r < 12; r++) {
		...
		for (let c = 0; c < 12; c++) {
			...
			else if (r >= 10 && ((c >= 2 && c <= 3) || (c >= 8 && c <= 9))) row.push("#553318");
			else row.push(null);
		}
		s.push(row);
	}
...
// ─── BFS PATHFINDING ────────────────────────────────
function bfs(sx: number, sy: number, ex: number, ey: number): { x: number; y: number }[] | null {
...
function V9SpeechBubbles() {
	...
	useEffect(() => {
		...
		function update(dt: number) {
			...
			characters.forEach((ch, ci) => {
				...
				if (ch.bubbleTimer <= 0) {
					...
					ch.bubbleTarget = 0;
					setTimeout(() => {
						ch.bubbleCycleIdx = (ch.bubbleCycleIdx + 1) % ACTION_CYCLE[ci].length;
				...
				if (ch.bubbleOpacity < ch.bubbleTarget) {
					ch.bubbleOpacity = Math.min(ch.bubbleTarget, ch.bubbleOpacity + fadeSpeed * dt);
				} else if (ch.bubbleOpacity > ch.bubbleTarget) {
					ch.bubbleOpacity = Math.max(ch.bubbleTarget, ch.bubbleOpacity - fadeSpeed * dt);
				}
				...
				if (ch.nextWalkTime <= 0 && ch.state !== "WALK") {
					...
					for (let r = 2; r < ROWS; r++)
						for (let c = 0; c < COLS; c++) if (WALKABLE[r][c]) targets.push({ x: c, y: r });
					if (targets.length) {
		...
		function render() {
			...
			// Walls
			for (let c = 0; c < COLS; c++)
				for (let r = 0; r < 2; r++) drawSprite(ctx, WALL_SPRITE, c * TILE, r * TILE);

			// Floor
			floorTiles.forEach((f) => drawSprite(ctx, f.sprite, f.tx * TILE, f.ty * TILE));
				...
				renderItems.push({
					...
					draw: () => {
						...
						if (ch.status !== "idle" && TOOL_ICONS[ch.toolIcon]) {
							drawSprite(ctx, TOOL_ICONS[ch.toolIcon], ch.x + 17, drawY + 4);
						}
						...
						drawStatusDot(ctx, labelX - labelW / 2 + 1, labelY - 2, ch.status, totalTime);
						...
						if (ch.bubbleOpacity > 0.01) {
							...
							drawBubble(ctx, ch.x + 8, drawY - 10, ch.action, isApproval, ch.bubbleOpacity);
						}
		...
		function handleClick(e: MouseEvent) {
			...
			characters.forEach((ch) => {
				const drawYY = ch.y - 8;
				if (mx >= ch.x && mx <= ch.x + 16 && my >= drawYY && my <= drawYY + 24) {
					clicked = ch;
				}
			});
