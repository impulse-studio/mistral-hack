...
const ROWS = 14;
const PX = 3;
const CELL = 16 * PX; // 48px per tile
...
// Door gap in wall
function drawDoorH(ctx: CanvasRenderingContext2D, tx: number, ty: number, floorFn: typeof drawWoodFloor) {
  floorFn(ctx, tx, ty);
  const bx = tx * 16;
  const by = ty * 16;
  // door frame sides
...
function drawDoorV(ctx: CanvasRenderingContext2D, tx: number, ty: number, floorFn: typeof drawWoodFloor) {
  floorFn(ctx, tx, ty);
  const bx = tx * 16;
  const by = ty * 16;
  px(ctx, bx, by, 16, 2, C.wallDark);
