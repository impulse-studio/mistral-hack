...
function makeBookshelf(variant: number): Sprite {
  ...
  for (let r = 0; r < 32; r++) {
    ...
    for (let c = 0; c < 16; c++) {
      ...
      else if (r === 10 || r === 11 || r === 20 || r === 21)
        row.push(C.shelfWood);
      else {
  ...
  for (let r = 0; r < 20; r++) {
    ...
    for (let c = 0; c < 32; c++) {
      ...
      // Cushions
        ...
        if (c === 0 || c === 31) row.push(C.couchC); // arms
        else if (c === 1 || c === 30) row.push(C.couchB);
        else if (c === 15 || c === 16) row.push(C.couchB); // center seam
...
function BreakRoomOffice() {
  ...
  useEffect(() => {
      ...
      state.characters.forEach((ch) => {
        ...
        if (ch.nextWalkTime <= 0 && ch.state !== "WALK") {
          ...
          for (let r = 2; r < ROWS; r++)
            for (let c = 0; c < COLS; c++)
              if (state.walkable[r]![c]) targets.push({ x: c, y: r });
          if (targets.length) {
    ...
    function render() {
      ...
      for (let c = 0; c < COLS; c++)
        for (let r = 0; r < 2; r++)
          drawSprite(state.wallSprite, c * TILE, r * TILE);

      // Floor
      state.floorTiles.forEach((f) =>
        drawSprite(f.sprite, f.tx * TILE, f.ty * TILE),
      );
    ...
    function frame(time: number) {
      const dt =
        state.lastTime === 0
          ? 0
          : Math.min((time - state.lastTime) / 1000, 0.1);
      state.lastTime = time;
      update(dt);
      render();
