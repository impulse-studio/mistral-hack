...
function drawDesk(ctx: CanvasRenderingContext2D, col: number, row: number, active: boolean) {
	const x = col * PX;
	const y = row * PX;
	...
	if (active) {
		...
		// Scanlines effect
		for (let i = 0; i < 7; i++) {
			drawPixelRect(ctx, mx + 3, my + 3 + i * 2, 22, 1, "rgba(0,0,0,0.15)");
		}
	...
	const handleCanvasClick = useCallback(
		(e: MouseEvent<HTMLCanvasElement>) => {
			...
			for (const agent of agents) {
				...
				if (!desk) continue;
				const [col, row] = desk;
				if (mx >= col * PX && mx < (col + 1) * PX && my >= row * PX && my < (row + 2) * PX) {
	...
	return (
		...
		>
			...
			>
				...
				>
					AI Office <span style={{ color: C.orange, fontSize: 14 }}>v3 — Mistral Orange</span>
				</h1>
				<button
					onClick={addAgent}
					style={{
